...
-- ##############################################################
-- This script adds 4-5 sample records for each table
--
-- INSTRUCTIONS:
...
-- Outreach 4: To Sarah Johnson - Reminder
INSERT INTO outreach_messages (
  contact_id,
  mail_item_id,
  message_type,
...
SELECT 
  ...
  mi.mail_item_id,
  'reminder',
  'Dear Sarah Johnson, Reminder: You have mail waiting at Mei Way Mail Plus. Mailbox: MB-104.',
  CURRENT_TIMESTAMP,
  'email'
FROM contacts c
JOIN mail_items mi ON c.contact_id = mi.contact_id
...
-- View all data (uncomment to run)

-- SELECT * FROM contacts WHERE user_id = 'YOUR_USER_ID_HERE';

-- SELECT mi.*, c.contact_person, c.mailbox_number 
-- FROM mail_items mi
-- JOIN contacts c ON mi.contact_id = c.contact_id
-- WHERE c.user_id = 'YOUR_USER_ID_HERE'
-- ORDER BY mi.received_date DESC;

-- SELECT * FROM message_templates WHERE user_id = 'YOUR_USER_ID_HERE';

-- SELECT om.*, c.contact_person, c.mailbox_number
-- FROM outreach_messages om
-- JOIN contacts c ON om.contact_id = c.contact_id
-- WHERE c.user_id = 'YOUR_USER_ID_HERE'
